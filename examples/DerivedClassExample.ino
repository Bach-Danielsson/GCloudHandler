...
class MyCloudHandler: public GCloudHandler {
public:
    MyCloudHandler(): GCloudHandler(IOT_PROJECT_ID, IOT_LOCATION
        , IOT_REGISTRY_ID, IOT_DEVICE_ID, IOT_PRIVATE_KEY) {};
    virtual ~MyCloudHandler() {};
    ...
    virtual void onConfigUpdate(String& config) {
        Serial.print("Cloud Configuration update received:");
        Serial.println(config);
    }
};

MyCloudHandler myCloudHander;
...
void loop() {
    ...
    if (millis - lastTelemetryUpdate > TELEMETRY_UPDATE_PERIOD)
        myCloudHander.publishTelemetry("{ \"some-value\": 1024 }");	
}
